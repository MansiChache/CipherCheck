...
export default function AdminRollcall({ room_id, rollcall, allParticipants, setRollCall }) {
    ...
    const [attendees, setAttendees] = useState(
        rollcall.attendees
            .filter(ele => allParticipants.find(p => p._id === ele))
            .map(ele => allParticipants.find(p => p._id === ele))
    );

    // Wrapped `getRemTime` inside useCallback to avoid function recreation on every render
    const getRemTime = useCallback(() => {
    ...
    // Wrapped `updateQR` inside useCallback to prevent unnecessary re-renders
    const updateQR = useCallback((qrText) => {
        QrCode.toDataURL(qrText, { width: 1000 }, (err, url) => {
            if (err) return;
            setQrCodeUrl(url);
            if (externalWindow.current) {
                externalWindow.current.document.querySelector("img").setAttribute("src", url);
            }
        });
    }, []);
    ...
    function showAbsentees() {
        let members = allParticipants.map(p => {
            ...
            p.rollcall = index >= 0 ? "P" : "A";
            return p;
        });

        members.sort((a, b) => {
            if (a.rollcall === b.rollcall) return a.uid - b.uid;
    ...
    }, [getRemTime, allParticipants, room_id, updateQR]);
...
function generateCSV(arr) {
    ...
    for (const row of arr) {
        string += `${row.uid},${row.fname + (row.lname || "")},${row.rollcall}\n`;
    }
